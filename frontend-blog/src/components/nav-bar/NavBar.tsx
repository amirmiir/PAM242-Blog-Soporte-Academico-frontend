...
const NavBar: FC = () => {
    ...
    return (
        <header className={`max-w-max-2xl mx-auto px-4 border-b-2 border-gray-300 bg-gray-100 h-[8vh] `}>
            <nav className="flex justify-between items-center h-full">
                ...
                <div className="flex h-full items-center font-semibold text-center">
                    ...
                    {
                        !isSmallScreen && (landingNavigation.map((item: navigationLabel, index: number) => (
                            <a key={index} href={item.to} className="text-gray-600 h-full hover:text-white hover:bg-red-500 px-4 place-content-center">{item.name}</a>
                        )))
                    }
